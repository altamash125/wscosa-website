            ...
            <div className="bg-black/40">
                <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-20">
                            ...
                            >
                                <div className="flex items-start gap-4">
                                    <div className="w-12 h-12 flex items-center justify-center rounded bg-amber-50 text-amber-600">
                                        {/* service-specific icon from ICONS map */}
                                        {ICONS[s.id] ?? (
                                            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M3 7h18M3 12h18M3 17h18"></path></svg>
                                        )}
                                    </div>
                                ...
                                {/* Hover-revealed Read more */}
                                <div className="absolute bottom-8 left-8">
                                    <Link
                                        href={`/services/${s.id}`}
                                        aria-label={`Read more about ${s.title}`}
                                        className="inline-flex items-center gap-2 text-amber-600 font-semibold focus:outline-none focus:ring-2 focus:ring-amber-300 rounded"
                                    >
                                        {/* Text animates in on hover or focus */}
                                        <span className="ml-1 max-w-0 overflow-hidden whitespace-nowrap opacity-0 transition-all duration-500 group-hover:max-w-xs group-hover:opacity-100 focus-visible:max-w-xs focus-visible:opacity-100">
                                            Read more
                                        </span>
                    ...
                    {/* <div className="mt-10 text-center">
                        <Link href="/constants.ts" className="inline-block bg-amber-600 text-white px-6 py-3 rounded-lg font-semibold hover:bg-amber-700 transition">
                            Show more
                        </Link>
                    </div> */}
                </div>
